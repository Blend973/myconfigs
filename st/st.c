...
int writing_output = 0;
int scroll_on_input = 0;

#define TSCREEN term.screen[IS_SET(MODE_ALTSCREEN)]
#define TLINEOFFSET(y)                                                         \
  (((y) + TSCREEN.cur - (writing_output ? 0 : TSCREEN.off) + TSCREEN.size) %     \
   TSCREEN.size)
...
static int selmaxrow(void);
static int selminrow(void);
...
void selsnap(int *x, int *y, int direction) {
  int minrow = selminrow(), maxrow = selmaxrow();
  int newx, newy, xt, yt;
  int delim, prevdelim;
  ...
  case SNAP_LINE:
    ...
    *x = (direction < 0) ? 0 : term.col - 1;
    if (direction < 0) {
      for (; *y > minrow; *y += direction) {
        if (!(TLINE(*y - 1)[term.col - 1].mode & ATTR_WRAP)) {
          break;
        }
      }
    } else if (direction > 0) {
      for (; *y < maxrow; *y += direction) {
...
int tscrollback(void) { return TSCREEN.off; }
...
void execsh(char *cmd, char **args) {
  ...
  /* Always use config.h shell as the primary option */
  sh = shell;
...
void tscrollup(int orig, int n) {
  ...
  /* If user is scrolled up, keep their position relative to the content */
  if (TSCREEN.off > 0 && orig == term.top) {
    TSCREEN.off += n;
...
void selscrollview(int n) {
  int minrow, maxrow;

  if (sel.ob.x == -1 || sel.alt != IS_SET(MODE_ALTSCREEN) || n == 0)
    return;
  ...
  minrow = selminrow();
  maxrow = selmaxrow();
  if (sel.ob.y < minrow || sel.oe.y < minrow || sel.ob.y > maxrow ||
      sel.oe.y > maxrow) {
    selclear();
    return;
  }
...
int selminrow(void) {
  int n = TSCREEN.size - term.row - TSCREEN.off;
...
int selmaxrow(void) { return term.row - 1 + TSCREEN.off; }

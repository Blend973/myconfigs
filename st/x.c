...
static int winhborderpx, winvborderpx;
...
static void mouselscrollreset(void);
static int mouselscrollstep(void);
static double mouselscrolltimeout(const struct timespec *);
static void mouselscrollupdate(XEvent *);
static int mouseselecttype(XEvent *);
static void timespecaddms(struct timespec *, long);
static void timeoutmin(double *, double);
...
static MouseSelScroll mouselscroll;
TermWindow win;
...
evcol(XEvent *e)
{
	int x = e->xbutton.x - winhborderpx;
	LIMIT(x, 0, win.tw - 1);
	return x / win.cw;
}
...
evrow(XEvent *e)
{
	int y = e->xbutton.y - winvborderpx;
	LIMIT(y, 0, win.th - 1);
	return y / win.ch;
}

static void
timespecaddms(struct timespec *ts, long ms)
{
	ts->tv_sec += ms / 1000;
...
static void
timeoutmin(double *timeout, double value)
{
	if (value < 0)
		return;
...
static void
mouselscrollreset(void)
{
	mouselscroll.active = 0;
	mouselscroll.dir = 0;
}
static void
mouselscrollupdate(XEvent *e)
{
	int y = e->xbutton.y - winvborderpx;

	if (!mouselscroll.active)
		return;

	mouselscroll.col = evcol(e);
	mouselscroll.type = mouseselecttype(e);
	if (y < 0) {
		mouselscroll.dir = -1;
		mouselscroll.row = 0;
	} else if (y >= win.th) {
		mouselscroll.dir = +1;
		mouselscroll.row = (win.th - 1) / win.ch;
	} else {
		mouselscroll.dir = 0;
		mouselscroll.row = evrow(e);
	}

	if (mouselscroll.dir != 0) {
		clock_gettime(CLOCK_MONOTONIC, &mouselscroll.next);
		timespecaddms(&mouselscroll.next, mouseselscrollinterval);
	}
}

static double
mouselscrolltimeout(const struct timespec *now)
{
	struct timespec current;

	if (!mouselscroll.active || mouselscroll.dir == 0)
		return -1;

	current = *now;
	return MAX(TIMEDIFF(mouselscroll.next, current), 0);
}

static int
mouselscrollstep(void)
{
	Arg arg = {.i = 1};
	int before, after;
	...
	selextend(mouselscroll.col, mouselscroll.row, mouselscroll.type, 0);
	clock_gettime(CLOCK_MONOTONIC, &mouselscroll.next);
	timespecaddms(&mouselscroll.next, mouseselscrollinterval);
...
{
	selextend(evcol(e), evrow(e), mouseselecttype(e), done);
	if (done)
...
cresize(int width, int height)
{
	...
	winhborderpx = borderpx + (win.w - 2 * borderpx - col * win.cw) / 2;
	winvborderpx = borderpx + (win.h - 2 * borderpx - row * win.ch) / 2;
...
{
	XRenderColor color = { .alpha = dc.bg_alpha };
...
xloadcols(void)
{
	...
	dc.col[defaultbg].color.alpha = dc.bg_alpha;
...
xinit(int cols, int rows)
{
   ...
   alpha_def = alpha;
	...
	winhborderpx = borderpx;
	winvborderpx = borderpx;
	/* adjust fixed window geometry */
	win.w = 2 * winhborderpx + cols * win.cw;
	win.h = 2 * winvborderpx + rows * win.ch;
	if (xw.gm & XNegative)
...
{
	float winx = winhborderpx + x * win.cw, winy = winvborderpx + y * win.ch, xp, yp;
...
{
	int winx = winhborderpx + x * win.cw, winy = winvborderpx + y * win.ch,
	...
	if (x == 0) {
		xclear(0, (y == 0)? 0 : winy, winhborderpx,
			((winy + win.ch >= winvborderpx + win.th)? win.h : (winy + win.ch)));
	}
	if (winx + width >= winhborderpx + win.tw) {
		xclear(winx + width, (y == 0)? 0 : winy, win.w,
			((winy + win.ch >= winvborderpx + win.th)? win.h : (winy + win.ch)));
	}
	if (y == 0)
		xclear(winx, 0, winx + width, winvborderpx);
	if (winy + win.ch >= winvborderpx + win.th)
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 4: /* Steady Underline */
			XftDrawRect(xw.draw, &drawcol,
					winhborderpx + cx * win.cw,
					winvborderpx + (cy + 1) * win.ch - \
						cursorthickness,
		...
		XftDrawRect(xw.draw, &drawcol,
				...
				1, win.ch - 1);
		XftDrawRect(xw.draw, &drawcol,
				winhborderpx + (cx + 1) * win.cw - 1,
				winvborderpx + cy * win.ch,
				1, win.ch - 1);
		XftDrawRect(xw.draw, &drawcol,
				winhborderpx + cx * win.cw,
				winvborderpx + (cy + 1) * win.ch - 1,
				win.cw, 1);
	}
}
...
xximspot(int x, int y)
{
	...
	xw.ime.spot.x = winhborderpx + x * win.cw;
	xw.ime.spot.y = winvborderpx + (y + 1) * win.ch;
...
run(void)
{
	...
	for (timeout = -1, drawing = 0, lastblink = (struct timespec){0};;) {
		...
		timeoutmin(&timeout, mouselscrolltimeout(&now));
		...
		if (mouselscrolltimeout(&now) == 0 && mouselscrollstep()) {
			xev = 1;
			clock_gettime(CLOCK_MONOTONIC, &now);
		}

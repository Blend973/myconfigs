...
# Robustness improvements: shlex escaping, cache cleanup, better regex, error handling.
...
CLI_VERSION = "0.4.6" # Bumped version for robustness update
CLI_AUTHOR = "Benexl"
...
def cleanup_cache():
    """Removes preview images older than 24 hours to prevent disk bloat."""
    try:
        now = time.time()
        cutoff = now - 86400 # 24 hours
        for filename in os.listdir(CLI_PREVIEW_IMAGES_CACHE_DIR):
            filepath = os.path.join(CLI_PREVIEW_IMAGES_CACHE_DIR, filename)
            if os.path.isfile(filepath):
                if os.path.getmtime(filepath) < cutoff:
                    os.remove(filepath)
        
        # Also clean up text previews
        for filename in os.listdir(CLI_PREVIEW_SCRIPTS_DIR):
            filepath = os.path.join(CLI_PREVIEW_SCRIPTS_DIR, filename)
            if os.path.isfile(filepath):
                if os.path.getmtime(filepath) < cutoff:
                    os.remove(filepath)
    except Exception:
...
"""
    ...
    # Robustness fix: Updated sed regex to handle numbers > 99 correctly
...
if [ "$MODE" = "video" ]; then
  title="$SELECTION"
  # Robustness: Use regex to strip leading numbers (e.g. "01 ", "100 ")
...
def load_config():
    ...
    init_pretty_print()
    create_bash_helpers()
    cleanup_cache() # Run cleanup on startup
...
def generate_text_preview(data):
    ...
    for i, video in enumerate(entries):
        ...
        # Robustness: Use shlex.quote to safely escape strings for Bash
        raw_title = video.get("title", "")
        # Remove leading numbers for hashing consistency
        clean_title = re.sub(r'^[0-9]+ ', '', raw_title)
        
        # Hash the clean title to match what the Bash script will calculate
        filename_hash = generate_sha256(clean_title)
        
        # Prepare display variables (escaped for shell)
        ...
        description = video.get("description") or "null"
        safe_description = shlex.quote(description)
        
        channel = video.get("channel", "")
        safe_channel = shlex.quote(channel)
        ...
        # Robustness: Using shlex.quote variables inside the f-string
        content = f"""
...
def update_json_list(filename, video, add=True, limit=None):
    ...
    if os.path.exists(filepath):
        try:
            ...
            # Robustness: Handle corrupted JSON files gracefully
            pass
    ...
    try:
        ...
        send_notification(f"Error writing to {filename}")
...
def playlist_explorer(search_results, url, urlForAll=None):
    ...
    while True:
        ...
        if selection == "Next":
            ...
            PLAYLIST_END -= int(CONFIG["NO_OF_SEARCH_RESULTS"])
            # Robustness: Ensure END doesn't drop below the page size

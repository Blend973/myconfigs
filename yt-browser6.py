...
DEFAULT_CONFIG = {
    ...
    "VIDEO_QUALITY": "1080", # Default bumped to 1080, will downscale if needed
...
def is_true(value):
    """Safely determine truthiness from string config."""
...
def check_dependencies():
    ...
    # Removed 'jq' as strictly parsing is done in Python
    required = ["yt-dlp", "fzf", "curl"] 
...
def send_notification(message):
    ...
    # Non-blocking sleep for notifications if possible, otherwise short blocking
    time.sleep(int(CONFIG.get("NOTIFICATION_DURATION", 1)))
...
def create_bash_helpers():
    ...
    # Improvements: Added error suppression to which checks, consistent quotes
...
fzf_preview() {{
  ...
  if [ "$IMAGE_RENDERER" = "icat" ] || [ -n "$KITTY_WINDOW_ID" ]; then
    ...
    # Fallback to text if no image renderer
...
def load_config():
    ...
    # Format browser args - Robust check to prevent duplication
    browser = CONFIG["PREFERRED_BROWSER"]
    if browser and "--cookies-from-browser" not in browser:
        # Check if browser is just a word like "firefox" or "chrome"
        if len(browser.split()) == 1:
            CONFIG["PREFERRED_BROWSER"] = f"--cookies-from-browser {browser}"
    ...
    PLAYLIST_END = int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
    create_bash_helpers()
    cleanup_cache()
...
def prompt(text, value=""):
    ...
    if CONFIG["PREFERRED_SELECTOR"] == "rofi":
        ...
        if is_true(CONFIG["SEARCH_HISTORY"]) and history_text:
...
def launcher(options_str, prompt_text, preview_mode=None):
    ...
    # Check if FZF is actually installed, fallback to input if not
    if selector == "fzf" and not shutil.which("fzf"):
        print("FZF not found. Falling back to basic input.")
        print("-" * 20)
        print(options_str)
        print("-" * 20)
...
def run_yt_dlp(url, extra_args=None):
    ...
    # Safely handle Preferred Browser configuration
    ...
    if shutil.which("gum"):
        # Gum spin adds a nice loading animation
        spin_cmd = ["gum", "spin", "--show-output", "--"] + cmd
        proc = subprocess.run(spin_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    else:
        sys.stderr.write("Fetching from YouTube...\n")
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    if proc.returncode != 0:
        # Don't show confusing JSON errors if the network fetch actually failed
        send_notification("Network error or invalid URL.")
        return None
    try:
        return json.loads(proc.stdout)
    except json.JSONDecodeError:
        # Robustness: Attempt to recover JSON from mixed output (yt-dlp sometimes warns before JSON)
        try:
            output = proc.stdout
...
def generate_text_preview(data):
    ...
    for i, video in enumerate(data["entries"]):
        ...
        view_count = "{:,}".format(int(vc)) if vc is not None and str(vc).isdigit() else "Unknown"
        ...
        # Use printf safe quoting logic later
        safe_description = shlex.quote(desc.replace('\n', ' ').replace('\r', ' '))
        safe_channel = shlex.quote(video.get("channel", "Unknown"))
        ...
        if dur:
            try:
                dur = float(dur)
                if dur >= 3600: duration_str = f"{int(dur // 3600)}h {int(dur % 3600 // 60)}m"
                elif dur >= 60: duration_str = f"{int(dur // 60)}m {int(dur % 60)}s"
                else: duration_str = f"{int(dur)}s"
            except: pass
        ...
        if ts:
            try:
                # Robustness: ts might be float or none
                diff = CURRENT_TIME - int(float(ts))
                if diff < 60: timestamp_str = "just now"
                elif diff < 3600: timestamp_str = f"{diff // 60} min ago"
                elif diff < 86400: timestamp_str = f"{diff // 3600} hr ago"
                elif diff < 604800: timestamp_str = f"{diff // 86400} days ago"
                elif diff < 2635200: timestamp_str = f"{diff // 604800} wks ago"
                elif diff < 31622400: timestamp_str = f"{diff // 2635200} mths ago"
                else: timestamp_str = f"{diff // 31622400} yrs ago"
            except: pass

        # Using printf instead of echo to prevent description flags (-e, -n) causing issues
        content = f"""
...
echo "Live:     {live_status}"
echo "Uploaded: {timestamp_str}"
ll=1
while [ $ll -le $FZF_PREVIEW_COLUMNS ];do echo -n -e "─" ;(( ll++ ));done;
echo
if [ {safe_description} != "null" ]; then printf "%s" {safe_description}; fi
"""
...
def download_preview_images(data, prefix=""):
    ...
    for video in data["entries"]:
        ...
        url = thumbs[-1]["url"] # Use largest thumb
        filename = generate_sha256(url)
        # Check if hash file exists to avoid re-download
    ...
    if entries_to_download:
        ...
        # Added --create-dirs for safety, though we ensure dir exists in config
        subprocess.Popen(["curl", "--create-dirs", "-s", "-K", previews_file], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
def playlist_explorer(search_results, url):
    ...
    audio_only_mode = is_true(CONFIG["AUDIO_ONLY_MODE"])
    autoplay_mode = CONFIG["AUTOPLAY_MODE"]

    download_images = False

    while True:
        # UX Improvement: Handle empty search results seamlessly
        if not search_results or "entries" not in search_results or not search_results["entries"]:
            print("No results found.")
            time.sleep(1)
            # If we are deep in a playlist, this means we hit the end
            if PLAYLIST_START > 1:
                # Revert logic
                PLAYLIST_START -= int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
                PLAYLIST_END -= int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
                # Refetch previous valid page
                search_results = run_yt_dlp(url)
                if not search_results: break # If even that fails, exit
            else:
                break
        ...
        if is_true(CONFIG["ENABLE_PREVIEW"]) and CONFIG["PREFERRED_SELECTOR"] == "fzf" and not download_images:
            download_preview_images(search_results)
            download_images = True

        # Generate Menu
        if is_true(CONFIG["ENABLE_PREVIEW"]):
            options_str = "\n".join(titles) + f"\nNext\nPrevious\nBack\nExit"
        ...
        # Strip ANSI colors returned by gum/rofi styling if any
        selection = re.sub(r'(?:\x1B[@-_]|[\x80-\x9F])[0-?]*[ -/]*[@-~]', '', selection)
        selection = re.sub(r'^[^0-9]*  ', '', selection).strip()
        clear_screen()
        ...
        if selection == "Next":
            ...
            PLAYLIST_END -= int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
            min_end = int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
        ...
        while True:
            ...
            if action_sel == "Watch":
                ...
                while True:
                    ...
                    if CONFIG["PLAYER"] == "mpv":
                        if audio_only_mode:
                            ...
                            # Stronger format selection for strict quality control
                            player_cmd.append(f"--ytdl-format=bestvideo[height<={q}]+bestaudio/best[height<={q}]/best")

                    elif CONFIG["PLAYER"] == "vlc":
                        # VLC doesn't support ytdl-format easily via CLI args, relies on internal config
                        player_cmd.extend(["--meta-title", clean_title])
                ...
                # Break action menu loop after watching to allow fresh selection or back
                break

            elif action_sel == "Download":
                folder = "audio" if audio_only_mode else "videos"
                
                # Robust Quality Selection for Download
                if audio_only_mode:
                    ext_args = ["-x", "-f", "bestaudio", "--audio-format", "mp3", "--add-metadata"]
                else:
                    q = CONFIG["VIDEO_QUALITY"]
                    ext_args = []
                    if q.isdigit():
                         ext_args = ["-f", f"bestvideo[height<={q}]+bestaudio/best[height<={q}]/best"]
                    ext_args.append("--embed-thumbnail")
                ...
                # Using start_new_session to prevent zombie processes or termination signals
                subprocess.Popen(cmd, start_new_session=True)
                send_notification(f"Download started: {clean_title}")

    # Reset playlist state when exiting explorer
    PLAYLIST_START = 1
    PLAYLIST_END = int(CONFIG.get("NO_OF_SEARCH_RESULTS", 30))
...
def main_menu(initial_action=None, search_term=None):
    ...
    if action == "Exit": byebye()
            ...
            if re.match(r'^![0-9]{1,2}$', search_term):
                ...
                if os.path.exists(hist_file):
                    try:
                        ...
                        # Deduplicate logic for unique history selection
                        lines = list(dict.fromkeys(lines))
                        if lines and idx <= len(lines): 
                            search_term = lines[-idx] # Fixed index logic
                    except Exception: pass
        ...
        if is_true(CONFIG["SEARCH_HISTORY"]):
            hist_file = os.path.join(CLI_CACHE_DIR, "search_history.txt")
            lines = []
            if os.path.exists(hist_file):
                try:

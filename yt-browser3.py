#!/usr/bin/env python3
# Optimized YT-Browser (Clean UI & Advanced Autoplay)
...
# Autoplay modes: "off", "playlist", "related"
AUTOPLAY_MODE = "off" 
...
def load_config():
    ...
    if not os.path.exists(config_file):
        with open(config_file, 'w') as f:
            f.write(f"EDITOR: {os.environ.get('EDITOR', 'nano')}\nPREFERRED_SELECTOR: fzf\n")
                ...
                while True:
                    ...
                    if AUTOPLAY_MODE != "off":
                        ...
                        if AUTOPLAY_MODE == "playlist":
                            ...
                            vid_id = video.get("id")
                            # Use YouTube Mix logic to find related
                            mix_url = f"https://www.youtube.com/watch?v={vid_id}&list=RD{vid_id}"
                            # Fetch only first few items to be fast
                            # We override global playlist start/end for this specific fetch
                            ...
                            try:
                                ...
                                if mix_data and "entries" in mix_data:
                                    for entry in mix_data["entries"]:
                                        # Find the first entry that isn't the current video
                        ...
                        # Non-blocking if autoplay is off
